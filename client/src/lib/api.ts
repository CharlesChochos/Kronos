...
export function useSavePersonalityProfile() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (profile: Partial<PersonalityProfile>) => {
      const res = await fetch("/api/personality-profile", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(profile),
      });
      if (!res.ok) throw new Error("Failed to save personality profile");
...
export function useAISuggestions(dealId?: string) {
  return useQuery({
    queryKey: dealId ? ["ai-suggestions", dealId] : ["ai-suggestions"],
    queryFn: async () => {
      const url = dealId ? `/api/deals/${dealId}/ai-suggestions` : "/api/ai-suggestions";
      const res = await fetch(url);
...
export function useRespondToAISuggestion() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, action }: { id: number; action: 'accept' | 'reject' }) => {
      const res = await fetch(`/api/ai-suggestions/${id}/respond`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
export function useScanEmails() {
  return useMutation({
    mutationFn: async (folderName: string = "Deals") => {
      const res = await fetch("/api/automation/scan-emails", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      return res.json() as Promise<{ processed: number; created: number; errors: number }>;
    },
  });
}
...
export function useEmailDeals() {
  return useQuery({
    queryKey: ["email-deals"],
    queryFn: async () => {
      const res = await fetch("/api/automation/email-deals");
      if (!res.ok) throw new Error("Failed to fetch email deals");
      return res.json() as Promise<EmailDeal[]>;
    },
  });
}
...
export function useGmailLabels() {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await fetch("/api/automation/gmail/labels");
...
export function useAutoAssignTeam() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (dealId: string) => {
      const res = await fetch(`/api/deals/${dealId}/auto-assign-team`, {
        method: "POST",
      });
...
export function useGenerateMilestones() {
  ...
  return useMutation({
    mutationFn: async ({ dealId, stage }: { dealId: string; stage?: string }) => {
      const res = await fetch(`/api/deals/${dealId}/generate-milestones`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    onSuccess: (_, vars) => {
      queryClient.invalidateQueries({ queryKey: ["deal-milestones", vars.dealId] });
    },
  });
}
...
export function useGenerateAISuggestions() {
  ...
  return useMutation({
    mutationFn: async (dealId: string) => {
      const res = await fetch(`/api/deals/${dealId}/generate-suggestions`, {
        method: "POST",
      });
      if (!res.ok) throw new Error("Failed to generate AI suggestions");
      return res.json();
    },
    onSuccess: (_, dealId) => {
      queryClient.invalidateQueries({ queryKey: ["ai-suggestions", dealId] });
    },
  });
}
