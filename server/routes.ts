...
Only include entries with both name AND company. Extract ALL rows.`
  ...
  // Gmail - List labels
  app.get("/api/automation/gmail/labels", requireCEO, async (req, res) => {
    try {
      ...
      res.status(500).json({ error: error.message || "Failed to list Gmail labels" });
    }
  });

  // Gmail - Scan folder for deals
  app.post("/api/automation/scan-emails", requireCEO, async (req, res) => {
    try {
      const { folderName = "Deals" } = req.body;
      const { processEmailFolder } = await import("./dealAutomation");
      const results = await processEmailFolder(folderName);
      res.json(results);
    } catch (error: any) {
      console.error("Error scanning emails:", error);
      res.status(500).json({ error: error.message || "Failed to scan emails" });
    }
  });

  // Get email deals (processed emails)
  app.get("/api/automation/email-deals", requireCEO, async (req, res) => {
    try {
      const { db } = await import("./storage");
      const { emailDeals } = await import("@shared/schema");
      const { desc } = await import("drizzle-orm");
      const deals = await db.select().from(emailDeals).orderBy(desc(emailDeals.createdAt));
      res.json(deals);
    } catch (error: any) {
      res.status(500).json({ error: error.message || "Failed to fetch email deals" });
    }
  });

  // Personality Profile - Get current user's profile
  app.get("/api/personality-profile", requireAuth, async (req, res) => {
    try {
      const user = req.user as any;
      const { db } = await import("./storage");
      const { personalityProfiles } = await import("@shared/schema");
      const { eq } = await import("drizzle-orm");
      const [profile] = await db.select().from(personalityProfiles).where(eq(personalityProfiles.userId, user.id));
  ...
  // Personality Profile - Get any user's profile (CEO only)
  app.get("/api/personality-profile/:userId", requireCEO, async (req, res) => {
    try {
      const { db } = await import("./storage");
      const { personalityProfiles } = await import("@shared/schema");
      const { eq } = await import("drizzle-orm");
      const [profile] = await db.select().from(personalityProfiles).where(eq(personalityProfiles.userId, req.params.userId));
      res.json(profile || null);
    } catch (error: any) {
      res.status(500).json({ error: error.message || "Failed to fetch profile" });
    }
  ...
  // Personality Profile - Save/update profile
  app.post("/api/personality-profile", requireAuth, async (req, res) => {
    try {
      const user = req.user as any;
      const { db } = await import("./storage");
      const { personalityProfiles } = await import("@shared/schema");
      ...
      const profileData = {
        ...
        preferredDealTypes: req.body.preferredDealTypes || [],
        preferredSectors: req.body.preferredSectors || [],
        experienceLevel: req.body.experienceLevel,
        leadershipStyle: req.body.leadershipStyle,
        workloadCapacity: req.body.workloadCapacity || 5,
      ...
      const [existing] = await db.select().from(personalityProfiles).where(eq(personalityProfiles.userId, user.id));
      
      if (existing) {
        const [updated] = await db.update(personalityProfiles)
          .set(profileData)
          .where(eq(personalityProfiles.userId, user.id))
          .returning();
        res.json(updated);
      } else {
        const [created] = await db.insert(personalityProfiles).values(profileData).returning();
        res.json(created);
      }
    } catch (error: any) {
  ...
  // Milestones - Get milestones for a deal
  app.get("/api/deals/:dealId/milestones", requireAuth, requireInternal, async (req, res) => {
    try {
      const { db } = await import("./storage");
      const { milestones } = await import("@shared/schema");
      const { eq } = await import("drizzle-orm");
      const dealMilestones = await db.select().from(milestones).where(eq(milestones.dealId, req.params.dealId));
      res.json(dealMilestones);
    } catch (error: any) {
  ...
  app.post("/api/deals/:dealId/milestones", requireAuth, requireInternal, async (req, res) => {
    try {
      const { db } = await import("./storage");
      const { milestones } = await import("@shared/schema");
  ...
  app.patch("/api/milestones/:id", requireAuth, requireInternal, async (req, res) => {
    try {
      const { db } = await import("./storage");
      const { milestones } = await import("@shared/schema");
      const { eq } = await import("drizzle-orm");
  ...
  // AI Task Suggestions - Get suggestions for a deal
  app.get("/api/deals/:dealId/ai-suggestions", requireAuth, requireInternal, async (req, res) => {
    try {
      const { db } = await import("./storage");
      const { aiTaskSuggestions } = await import("@shared/schema");
      const { eq, desc } = await import("drizzle-orm");
      const suggestions = await db.select().from(aiTaskSuggestions)
        .where(eq(aiTaskSuggestions.dealId, req.params.dealId))
  ...
  // AI Task Suggestions - Get all pending suggestions
  app.get("/api/ai-suggestions", requireAuth, requireInternal, async (req, res) => {
    try {
      const { db } = await import("./storage");
      const { aiTaskSuggestions } = await import("@shared/schema");
      const { eq, desc } = await import("drizzle-orm");
      const suggestions = await db.select().from(aiTaskSuggestions)
        .where(eq(aiTaskSuggestions.status, 'pending'))
        .orderBy(desc(aiTaskSuggestions.createdAt));
      res.json(suggestions);
    } catch (error: any) {
      res.status(500).json({ error: error.message || "Failed to fetch AI suggestions" });
  ...
  // AI Task Suggestions - Accept/Reject suggestion
  app.post("/api/ai-suggestions/:id/respond", requireAuth, requireInternal, async (req, res) => {
    try {
      const user = req.user as any;
      ...
      const [updated] = await db.update(aiTaskSuggestions)
        .set({
          status: action === 'accept' ? 'accepted' : 'rejected',
          respondedBy: user.id,
          respondedAt: new Date(),
        })
        .where(eq(aiTaskSuggestions.id, req.params.id))
        .returning();
      
      // If accepted, apply the suggested changes
      if (action === 'accept' && updated.taskId && updated.suggestedChanges) {
        const { tasks } = await import("@shared/schema");
        const changes = updated.suggestedChanges as any;
        await db.update(tasks)
          .set({
            assignedTo: changes.newAssignee || undefined,
            priority: changes.newPriority || undefined,
            dueDate: changes.newDueDate || undefined,
          })
      ...
      res.status(500).json({ error: error.message || "Failed to respond to suggestion" });
    }
  });

  // Generate AI suggestions for a deal
  app.post("/api/deals/:dealId/generate-suggestions", requireCEO, async (req, res) => {
    try {
      const { generateTaskSuggestions } = await import("./dealAutomation");
      await generateTaskSuggestions(req.params.dealId);
      res.json({ success: true, message: "Suggestions generated" });
    } catch (error: any) {
      res.status(500).json({ error: error.message || "Failed to generate suggestions" });
    }
  });

  // Create milestones for a deal stage
  app.post("/api/deals/:dealId/generate-milestones", requireCEO, async (req, res) => {
    try {
      const { stage = 'Origination' } = req.body;
      const { createMilestonesForDeal } = await import("./dealAutomation");
      await createMilestonesForDeal(req.params.dealId, stage);
      res.json({ success: true, message: "Milestones created" });
    } catch (error: any) {
      res.status(500).json({ error: error.message || "Failed to create milestones" });
    }
  });

  // Auto-assign team to a deal
  app.post("/api/deals/:dealId/auto-assign-team", requireCEO, async (req, res) => {
    try {
      const { selectOptimalTeam } = await import("./dealAutomation");
      const deal = await storage.getDeal(req.params.dealId);
      if (!deal) {
        return res.status(404).json({ error: "Deal not found" });
      }
      ...
      const dealInfo = {
        name: deal.name,
        dealType: deal.dealType as 'M&A' | 'Capital Raising' | 'Asset Management',
        client: deal.client,
        sector: deal.sector,
      ...
      const team = await selectOptimalTeam(dealInfo, deal.stage);
      
      // Update deal with new team
      await storage.updateDeal(req.params.dealId, {
        lead: team.lead.name,
        podTeam: [team.lead, ...team.members],
      });
      
      res.json({ lead: team.lead, members: team.members });
    } catch (error: any) {
      res.status(500).json({ error: error.message || "Failed to auto-assign team" });
    }
  });

  // Workflow settings
  app.get("/api/workflow-settings", requireCEO, async (req, res) => {
    try {
      const { db } = await import("./storage");
      const { workflowSettings } = await import("@shared/schema");
      const settings = await db.select().from(workflowSettings);
  ...
  app.post("/api/workflow-settings", requireCEO, async (req, res) => {
    try {
      const user = req.user as any;
      ...
      const [existing] = await db.select().from(workflowSettings).where(eq(workflowSettings.settingKey, key));
      
      if (existing) {
        await db.update(workflowSettings)
          .set({ settingValue: value, updatedBy: user.id, updatedAt: new Date() })
          .where(eq(workflowSettings.settingKey, key));
      } else {
